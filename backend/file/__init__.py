...
async def handle_file_message(websocket, data, user_session):
    ...
    if data["type"] == "file-upload-start":
        ...
        if data['path'] == "tree/current_handling_files/":
            return
        ...
        if data['from'] == "tree/current_handling_files/":
            return
        await handle_file_move(websocket, data, user_session)
        # 同时更新tree和explorer
        await update_display(websocket, user_session, ["tree/", "expl/"])
    ...
    # 文件取消选择请求
        ...
        if user_session["current_file_selection"] == "tree/current_handling_files/":
            return
    ...
    elif data.get("type") == "file-explorer-enter-dir":
        # 处理进入子目录
        user_session["current_viewing_directory"] = data["dir_name"] + "/"
        await update_display(websocket, user_session, ["expl/"])
