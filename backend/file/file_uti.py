...
async def update_display(websocket, user_session, envolved_files):
    ...
    if "expl/" in all_prefix:
        # 获取当前explorer路径
        current_explorer_path = user_session.get("current_viewing_directory", "expl/")
        real_path = to_real_path(current_explorer_path, user_session)
        items = [current_explorer_path + i for i in os.listdir(real_path)]
        await websocket.send(json.dumps({
            "type": "file-display_expl",
            "root": current_explorer_path,
            "items": items
        }))
