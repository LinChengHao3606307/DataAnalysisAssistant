...
from backend.file.file_uti import update_display
import uuid
import json
from others.const import MAIN_MODEL
from backend.chat.workflows.conversation_handlers import ConvHandler


async def work_on(websocket, user_session:dict):
    """处理用户消息的完整流程"""
    history:History = user_session["history"]
    ...
    while True:
        ...
        print(full_prompt)
        response = await ask_llm(MAIN_MODEL, full_prompt, msg_id, websocket)
